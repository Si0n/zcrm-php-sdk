...
class ZohoOAuthPersistenceHandler implements ZohoOAuthPersistenceInterface
{
    public function saveOAuthData($zohoOAuthTokens)
    {
        ...
        try {
            ...
            if (!$result) {
                LogManager::severe('OAuth token insertion failed: (' . $db_link->errno . ') ' . $db_link->error);
            }
        } catch (\Exception $ex) {
            LogManager::severe("Exception occured while inserting OAuthTokens into DB(file::ZohoOAuthPersistenceHandler)({$ex->getMessage()})\n{$ex}");
        } finally {
            if (null != $db_link) {
                $db_link->close();
            }
    ...
    public function getOAuthTokens($userEmailId)
    {
        ...
        try {
            ...
            if (!$resultSet) {
                LogManager::severe('Getting result set failed: (' . $db_link->errno . ') ' . $db_link->error);
            ...
            LogManager::severe("Exception occured while getting OAuthTokens from DB(file::ZohoOAuthPersistenceHandler)({$ex->getMessage()})\n{$ex}");
        } finally {
            if (null != $db_link) {
                $db_link->close();
            }
    ...
    public function deleteOAuthTokens($userEmailId)
    {
        ...
        try {
            ...
            $resultSet = mysqli_query($db_link, $query);
            if (!$resultSet) {
                LogManager::severe('Deleting  oauthtokens failed: (' . $db_link->errno . ') ' . $db_link->error);
            }
        } catch (\Exception $ex) {
            LogManager::severe("Exception occured while Deleting OAuthTokens from DB(file::ZohoOAuthPersistenceHandler)({$ex->getMessage()})\n{$ex}");
        } finally {
            if (null != $db_link) {
                $db_link->close();
            }
    ...
    public function getMysqlConnection()
    {
        ...
        if ($mysqli_con->connect_errno) {
            LogManager::severe('Failed to connect to MySQL: (' . $mysqli_con->connect_errno . ') ' . $mysqli_con->connect_error);
