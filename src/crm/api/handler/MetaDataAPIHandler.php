...
class MetaDataAPIHandler extends APIHandler
{
    ...
    public function getZCRMModule($moduleDetails)
    {
        ...
        if (array_key_exists('fields', $moduleDetails) && null != $moduleDetails['fields']) {
        ...
        if (array_key_exists('related_list_properties', $moduleDetails) && null != $moduleDetails['related_list_properties']) {
            $crmModuleInstance->setRelatedListProperties(self::getRelatedListProperties($moduleDetails['related_list_properties']));
        }

        if (array_key_exists('$properties', $moduleDetails) && null != $moduleDetails['$properties']) {
            $crmModuleInstance->setProperties($moduleDetails['$properties']);
        }

        if (array_key_exists('per_page', $moduleDetails) && null != $moduleDetails['per_page']) {
            $crmModuleInstance->setPerPage($moduleDetails['per_page'] + 0);
        }

        if (array_key_exists('search_layout_fields', $moduleDetails) && null != $moduleDetails['search_layout_fields']) {
            $crmModuleInstance->setSearchLayoutFields($moduleDetails['search_layout_fields']);
        }
        if (array_key_exists('custom_view', $moduleDetails) && null != $moduleDetails['custom_view']) {
        ...
        if (array_key_exists('territory', $moduleDetails) && null != $moduleDetails['territory']) {
